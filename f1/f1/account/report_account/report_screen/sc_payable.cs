...
{
    public partial class sc_payable : DevExpress.XtraEditors.XtraForm
    {
        public sc_payable()
        {
            InitializeComponent();
        ...
        {
            dgv.Rows.Clear();
            string query = "select pay_hd_id,type,amount,note,date_ from pay_hd where date_  between '" + date1.Value.ToString("yyyy/MM/dd") + "' and '" + date2.Value.ToString("yyyy/MM/dd") + "'";
        ...
        private void dgv_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 1)
            {
                if (db.GetData("select isnull(max(pay_hd_id),0) from pay_hd where pay_hd_id='" + dgv.CurrentRow.Cells[1].Value + "" + "'").Rows[0][0] + "" != "0")
                {
